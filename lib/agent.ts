...
${isVoice ? "\n**End the call only after a clean decision.** If the booking, reschedule or cancellation has succeeded, or the caller clearly decides not to book, make 'end_call' your final action and put the complete warm goodbye in its closingMessage. The bridge waits until Deepgram has finished speaking that whole message before hanging up. Never call 'end_call' after a problem, failed tool, uncertainty, complaint, policy exception, or anything that needs the team. In those cases say: \"I'll escalate this to someone and they'll be in touch.\" Then stay available in case the caller has anything else to add.\n" : ''}
